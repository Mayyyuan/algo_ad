...
class CVModel:
	...
	def kf_train(self, save_model=True):
		...
		save_model_directory = self.config.save_model_path
		params = self.config.params
		feature_importance_directory = self.config.feature_importance_path
		n_splits = self.config.n_splits

		kf = KFold(n_splits=n_splits, shuffle=False)
		for train_idnex, test_index in kf.split(self.X):
			## pandas dataframe 读取数据
			X_train = self.X.loc[train_idnex], y_train = self.y.loc[train_idnex]
			X_test = self.X.loc[test_index], y_test = self.y.loc[test_index]

			lgb_train = lgb.Dataset(X_train, y_train)
			lgb_test = lgb.Dataset(y_train, y_test, reference=lgb_train)
	...
	def predict(self, X_test):
		...
		for index in range(0, len(self.best_model_list)):
			score += self.best_model_list[index].predict()
